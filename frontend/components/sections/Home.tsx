...
export default function Home({ onNavigate }: HomeProps) {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-[5vw] pt-32 bg-[radial-gradient(ellipse_at_50%_20%,rgba(139,92,246,0.10),transparent_60%)]">
      ...
      <h1 className="font-display font-black tracking-wide text-[clamp(3.38rem,10.4vw,7.28rem)]">
      ...
      <div className="mt-14 flex gap-6 font-ui text-sm">
        <button onClick={() => onNavigate("about")} className="text-ink-dim hover:text-purple transition-colors">
          about
        </button>
        <button onClick={() => onNavigate("projects")} className="text-ink-dim hover:text-purple transition-colors">
          projects
        </button>
        <button onClick={() => onNavigate("contact")} className="text-ink-dim hover:text-purple transition-colors">
          contact
        </button>
      </div>
    </section>
