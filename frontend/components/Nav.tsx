...
const BASE_LINKS: { id: Tab; label: string }[] = [
  { id: "home", label: "Home" },
...
export default function Nav({ active, onSelect, showAdmin }: NavProps) {
  const [open, setOpen] = useState(false);
  const links = showAdmin ? [...BASE_LINKS, { id: "admin" as Tab, label: "Admin" }] : BASE_LINKS;
      ...
      <nav>
        ...
        >
          {links.map((link) => {
            ...
            const isTerminal = link.id === "terminal";
            ...
            return (
              <li key={link.id}>
                <button onClick={() => select(link.id)} className={className}>
                  {prefix}
                  {link.label}
                </button>
              </li>
